...
public class TriForceLevels extends Game implements InputProcessor{

	TbSkin tbSkin;
	Stage stage;
	TextButton TbEasy,TbMeduim,TbHard,TbImposs;
	@Override
	public void create() {
		stage = new Stage();
		tbSkin= new TbSkin();
		Skin skin =tbSkin.getSkin();//calls skin
		TbEasy= new TbText("Easy", skin);
		TbEasy.setPosition(Gdx.graphics.getWidth() / 2 - Gdx.graphics.getWidth() / 8, Gdx.graphics.getHeight() / 3*2);
		TbMeduim= new TbText("Medium",skin);
		TbMeduim.setPosition(Gdx.graphics.getWidth() / 2 - Gdx.graphics.getWidth() / 8, Gdx.graphics.getHeight() / 2 );
